		...
		if (!_selectionChanging) {
			...
			try {
				if (event.getSource() == _browser) {
					// String keyPath = (String) ((IStructuredSelection)
					// event.getSelection()).getFirstElement();
					// IApiBinding apiBinding = (IApiBinding)
					// ((IStructuredSelection)
					// _inspector.getBindingsTableViewer().getSelection()).getFirstElement();
					// if (apiBinding != null) {
					// RefactoringWodElement element =
					// _inspector.getRefactoringElement();
					// if (element != null) {
					// try {
					// element.setValueForBinding(keyPath,
					// apiBinding.getName());
					// } catch (Exception e) {
					// e.printStackTrace();
					// }
