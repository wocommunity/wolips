...
/**
 * This class is an exact copy of {@link ToolTip}.
 * The only thing I changed is the method {@code toolTipHide()} from private to protected,
 * so that it's Override able.
 *
 * @author Savas Celik
...
public abstract class CustomToolTip {
	 ...
	 * @see #NO_RECREATE
	 */
	public CustomToolTip(Control control, int style, boolean manualActivation) {
		this.control = control;
		this.style = style;
		this.listener = new ToolTipOwnerControlListener();
		this.shellListener = event -> {
			if (CustomToolTip.this.control != null
					&& !CustomToolTip.this.control.isDisposed()) {
				CustomToolTip.this.control.getDisplay().asyncExec(() -> {
					// Check if the new active shell is the tooltip
					// itself
					if (CustomToolTip.this.control != null && !CustomToolTip.this.control.isDisposed()
							&& CustomToolTip.this.control.getDisplay().getActiveShell() != CURRENT_TOOLTIP) {
	 ...
	 */
	protected boolean shouldHideToolTip(Event event) {
