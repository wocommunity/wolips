...
/**
 * IHateCCombo fixes a visual bug that you see fairly frequently
 * on OS X.
 * 
 ...
 */
public final class IHateCCombo extends Composite {
 ...
 * @see Widget#getStyle()
 */
public IHateCCombo (Composite parent, int style) {
	...
	listener = new Listener () {
		public void handleEvent (Event event) {
			...
			if (IHateCCombo.this == event.widget) {
				comboEvent (event);
				return;
			}
	...
	filter = new Listener() {
		public void handleEvent(Event event) {
			...
			if (shell == IHateCCombo.this.getShell ()) {
				handleFocus (SWT.FocusOut);
			}
		}
	};
...
void dropDown (boolean drop) {
	...
	if (index != -1) {
		if (index < visibleItemCount / 2) {
			index = 0;
		}
		else {
			index -= visibleItemCount / 2;
		}
		index = Math.min(index, list.getItemCount() - itemCount);
...
void internalLayout (boolean changed) {
	...
	text.setBounds (0, 1, width - arrowSize.x - 5, height - 1);
	...
	switch (event.type) {
		...
		case SWT.KeyDown: {
			...
			if (event.keyCode == SWT.CR) {
				dropDown (false);
				notifyListeners (SWT.DefaultSelection, keyEvent);
			}
