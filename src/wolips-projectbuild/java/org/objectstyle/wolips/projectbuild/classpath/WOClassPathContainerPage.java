  ...
  implements IClasspathContainerPage 
{
	...
	public void createControl(Composite parent)  {
	    ...
	    _uiList = new List (thisPage, SWT.MULTI | SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL);
	    GridData gd = new GridData (GridData.HORIZONTAL_ALIGN_FILL|GridData.FILL_HORIZONTAL); //|GridData.VERTICAL_ALIGN_FILL
	    Rectangle trim = _uiList.computeTrim(0,0,0,12*_uiList.getItemHeight());
	    gd.heightHint = trim.height;
	    _uiList.setLayoutData(gd);
	
	    //ScrolledComposite scroller = new ScrolledComposite (thisPage, SWT.H_SCROLL|SWT.V_SCROLL);
	    //scroller.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
	    //scroller.setContent(_uiList);
	    //scroller.setAlwaysShowScrollBars(true);
	    //scroller.setSize(SWT.DEFAULT, 350);
	...
	public boolean finish()  {
	    ...
	    String [] selected = _uiList.getSelection();
	    for (int i = 0; i < selected.length; ++i) {
	      path = path.append (selected[i]);
	    }
  ...
  private void _initFrameworkList () {
	...
	if (fwBase.exists() && fwBase.isDirectory()) {
	  ...
	  for (int i = 0; i < frameworks.length; ++i) {
		...
		_uiList.add(thisOne.substring(0, thisOne.length()-10)); // cut off the .framework
	  }
