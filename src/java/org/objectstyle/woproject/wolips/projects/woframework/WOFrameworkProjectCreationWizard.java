...
public class WOFrameworkProjectCreationWizard extends BasicNewResourceWizard implements INewWizard, IExecutableExtension {

	private WOFrameworkProjectCreationWizardPage[] fPages;
	private IConfigurationElement fConfigElement;

	public WOFrameworkProjectCreationWizard() {
		super();
		setDialogSettings(WOLipsPlugin.getDefault().getDialogSettings());
		setWindowTitle(WOFrameworkProjectMessages.getString("WOFrameworkProjectCreationWizard.title"));		 //$NON-NLS-1$
	...
	public void addPages() {
		...
		fPages=  new WOFrameworkProjectCreationWizardPage[children.length];
		
		for (int i= 0; i < children.length; i++) {
			IConfigurationElement curr= children[i];
			fPages[i]= new WOFrameworkProjectCreationWizardPage(i, children[i]);
			addPage(fPages[i]);
		}
	}
	...
	public boolean performFinish() {
		WOFrameworkProjectCreationOperation runnable= new WOFrameworkProjectCreationOperation(fPages, new ImportOverwriteQuery());
	...
	private void handleException(Throwable target) {
		String title= WOFrameworkProjectMessages.getString("WOFrameworkProjectCreationWizard.op_error.title"); //$NON-NLS-1$
		String message= WOFrameworkProjectMessages.getString("WOFrameworkProjectCreationWizard.op_error.message"); //$NON-NLS-1$
	...
	private class ImportOverwriteQuery implements IOverwriteQuery {
		...
		private int openDialog(final String file) {
			...
			getShell().getDisplay().syncExec(new Runnable() {
				public void run() {
					String title= WOFrameworkProjectMessages.getString("WOFrameworkProjectCreationWizard.overwritequery.title"); //$NON-NLS-1$
					String msg= WOFrameworkProjectMessages.getFormattedString("WOFrameworkProjectCreationWizard.overwritequery.message", file); //$NON-NLS-1$
